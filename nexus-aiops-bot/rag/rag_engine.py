...
class LogRageEngine:
    ...
    def run_query(self, query: str, time_window_mins: int = 30, image_base64: str = None) -> dict:
        ...
        system_prompt = (
            ...
            "   Your diagrams must look highly professional, polished, and structured (similar to premium templates). Follow these design guidelines:\n"
            "   - **VPC / Subnet Bounding Boxes**: Draw large dashed-line rectangles (`\"strokeColor\": \"#94a3b8\", \"strokeStyle\": \"dashed\"`) to represent virtual networks or subnets (e.g. 'Virtual Network A (10.1.0.0/16)').\n"
            "   - **Grouped Resources**: Arrange resources (like VMs, DBs) neatly inside their respective subnet bounding boxes.\n"
            "   - **Color Coding**: Use a cohesive, premium color palette for elements:\n"
            "     * Load Balancer / Ingress: Purple (`#8b5cf6` / `#c084fc` background)\n"
            "     * Compute Instances (VMs/Pods): Blue (`#3b82f6` / `#93c5fd` background)\n"
            "     * Databases (RDS/SQL): Teal (`#0d9488` / `#5eead4` background) or Green\n"
            "     * Bounding Boxes / Group borders: Slate/Gray\n"
            "   - **Text Labels**: Every node must have a corresponding 'text' element placed accurately inside or below it.\n"
            "   - **Labeled Connections**: Use arrows to connect nodes indicating traffic flow, and place a descriptive text label near the arrow (e.g., 'VNet Peering (Bi-directional)').\n"
            "   - **Key Metrics / Details Sidebar**: Draw a sidebar rectangle containing bullet points of key details (e.g., 'Uses Private IPs', 'Bi-directional') to make the diagram self-explanatory.\n\n"
            ...
            "     ]\n"
            "   }}\n"
            "   Ensure the nodes are connected, structured horizontally or vertically, and arranged with clean offsets to avoid overlaps.\n\n"
